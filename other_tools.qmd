...
A spatial join transfers attributes from one dataset to another based on spatial relationships. Unlike a traditional table join, which relies on matching attribute values, a spatial join links records based on location (intersection or distance)
...
This tool creates a raster surface by estimating values at unsampled locations based on nearby known data points. Different methods (e.g., Inverse Distance Weighting, Kriging, Spline) use the values of surrounding points to predict continuous variation across space. The result is a smooth surface that represents a gradual change in the variable of interest.

Example: You have air quality monitors spread across a city. By interpolating their measurements, you can generate a continuous surface showing estimated pollution levels everywhere in the city, even in places without sensors.
