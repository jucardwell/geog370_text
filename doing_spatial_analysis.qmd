...
This chapter introduces common spatial analysis tools and demonstrates how to use them in QGIS. While spatial questions often require combining multiple tools in sequence, you are not expected to design these workflows from scratch. Instead, the focus here is on learning to apply specific tools to explore data, interpret results, and better understand geographic patterns.
...
**Selecting by attribute allows you to filter all of the records in a dataset based on the values in one of the variables. Features that meet the criteria will become "selected"**
...
**Reprojecting data allows you to convert a GCS or PCS to a different GCS or PCS. You would do this to o match the coordinate system used by other datasets in your project, or to enable specific types of spatial analysis that require a projected coordinate system (such as calculating area or distance).**
...
**A buffer is a new area generated around a point, line, or polygon feature to identify everything within a certain distance**
...
**An intersect identifies the spatial overlap between two or more layers, creating a new dataset that contains only the features or portions of features where the layers intersect.** 
...
**A clip trims one layer using the boundaries of another, keeping only the features (or parts of features) that fall within the clip layer’s extent.** 
...
**A service area represents the geographic region that can be reached from a location within a given travel distance or time, often used to model accessibility.**
...
**A spatial join transfers attribute data from one layer to another based on their spatial relationship (like containment, intersection, or proximity).**
...
**Calculate slope determines the steepness or incline of a surface, usually from a digital elevation model (DEM), expressed in degrees or percent rise.** 
...
**Reclassifying a raster means assigning new values to ranges or categories of existing raster cell values. It is often used to simplify data, group values into categories, or prepare a raster for analysis like suitability modeling.**
...
**Raster math refers to performing mathematical operations on raster layers, cell-by-cell. It allows you to combine, compare, or transform raster datasets using arithmetic expressions (e.g., addition, subtraction, multiplication, division).**
...
**Zonal statistics calculates summary statistics (like mean, sum, max, or standard deviation) for raster values within the boundaries of defined zones from another dataset—typically a polygon layer.**
